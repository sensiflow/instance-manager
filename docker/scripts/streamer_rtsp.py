...
from streamer_interface import StreamerInterface


class StreamerRTSP(StreamerInterface):

    def __init__(self, destination_uri: str, width: int, height: int, framerate: int = 30):
        self.proc = None
        self.pipe = None
    ...
    def start_stream(self):
        ...
        command = [
            ...
            '-stream_loop', '-1',
            '-f', 'rawvideo',
            '-vcodec', 'rawvideo',  # because we dont know what type of codec is in the input
            ...
            '-preset', 'ultrafast',
            '-f', 'rtsp',
            '-rtsp_transport', 'tcp',  # TODO: test with udp and see if its better
